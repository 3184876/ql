...
download_repo() {
    ...
    rm -rf /root/pm >>/dev/null 2>&1
    git clone https://github.com/TeamPGM/PagerMaid-Pyro.git /root/pm >>/dev/null 2>&1
    cd /root/pm >>/dev/null 2>&1
    echo "Hello World!" >/root/pm/public.lock
}
...
login_screen() {
    ...
    screen -dmS userbot
    sleep 1
    screen -x -S userbot -p 0 -X stuff "cd /root/pm && $PYV -m pagermaid"
    screen -x -S userbot -p 0 -X stuff $'\n'
    sleep 3
    if [ "$(ps -def | grep [p]agermaid | grep -v grep)" == "" ]; then
        echo "PagerMaid 运行时发生错误，错误信息："
        cd /root/pm && $PYV -m pagermaid >err.log
        cat err.log
        screen -S userbot -X quit >>/dev/null 2>&1
        exit 1
    fi
...
cleansession() {
    ...
    systemctl stop pagermaid >>/dev/null 2>&1
    echo "正在删除账户授权文件 . . ."
    rm -rf /root/pm/pagermaid.session >>/dev/null 2>&1
    echo "请进行重新登陆. . ."
    if [ "$release" = "centos" ]; then
        yum_python_check
        yum_screen_check
